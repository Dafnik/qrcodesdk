...
`QRCodeCanvasOptions` and `QRCodeImageOptions` combine their corresponding renderer options with the shared core matrix options. Framework components consume these canonical option types directly.
