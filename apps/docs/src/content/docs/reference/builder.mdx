...
description: Reference for QRCodeSDK payload, modes, ECI, matrix options, builder methods, and matrix output.
docType: reference
...
`qrcode()` creates an immutable builder from `@qrcodesdk/core`. Each options method returns a
...
qrcode(1234567890);
qrcode().payload('payload supplied later');
```

QRCodeSDK generates QR Code Model 2 symbols. It supports numeric, alphanumeric, and UTF-8 octet payloads,
including automatic mixed-mode segmentation.

## Encoding modes

| Mode           | Accepted payload              | Use it for                                     |
| -------------- | ----------------------------- | ---------------------------------------------- |
...
| omitted        | Any supported payload         | Automatic shortest-bit segmentation; default   |
...
const equivalent = qrcode('Grüße ✅').options({
  mode: 'octet',
  eci: true,
...
larger version. For center images, see the reliability guidance in
...
When a pinned version cannot contain the payload, generation throws `QRCode: Payload too large`.
...
| -------------------------- | ------------------------- | -------------------------------------------------------------- |
| `.payload(value)`          | a builder with a payload  | Sets or replaces the payload                                   |
| `.options(options?)`       | a configured builder      | Merges all supplied matrix options                             |
...
| `.matrix()`                | `QRCodeMatrix`            | Generates the matrix; requires a payload                       |
...
The builder's types prevent `.matrix()` or `.render()` before a payload is set, and prevent `.render()`
