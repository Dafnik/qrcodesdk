...
function getEyeDropperConstructor(): EyeDropperConstructor | null {
  return (
    (globalThis as typeof globalThis & {EyeDropper?: EyeDropperConstructor}).EyeDropper ?? null
  );
}
  ...
  template: `
    @if (showEyeDropper()) {
      <button
        ...
        <ng-icon class="text-muted-foreground/80" name="lucidePipette" aria-hidden="true" />
      </button>
    }
      ...
      <div class="grid gap-2">
        ...
        @if (showOpacity()) {
          <div
            ...
            <span
              class="ring-ring absolute block size-4 rounded-full border-3 border-white shadow-md outline-none focus-visible:ring-2"
              [style.backgroundColor]="opacityThumbColor()"
              brnSliderThumb></span>
          </div>
        }
      </div>
...
export class ColorAdjustmentControlsComponent {
  ...
  protected setOpacity(values: number[]): void {
    this.picker.commit({
      ...this.picker.state(),
      a: (values[0] ?? this.picker.state().a * 100) / 100,
    });
  }
}
