...
@Component({
  ...
  template: `
    @for (row of colors(); track $index; let rowIndex = $index) {
      <div class="grid grid-cols-[repeat(var(--palette-columns),minmax(0,1fr))]" role="row">
        @for (color of row; track $index; let columnIndex = $index) {
          <button
            ...
            type="button"
            role="gridcell"
            data-slot="color-palette-swatch"></button>
        }
      </div>
    }
  `,
...
export class ColorPaletteComponent {
  ...
  protected readonly paletteAspectRatio = computed(
    () => `${this.columnCount()} / ${this.colors().length}`,
  );
