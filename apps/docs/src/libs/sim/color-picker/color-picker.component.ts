...
import {type ColorPickerContext, provideColorPicker} from './color-picker.token';
...
export class ColorPickerComponent implements ColorPickerContext {
  ...
  public readonly state = linkedSignal<string, ColorPickerHsva>({
    ...
    computation: (value, previous) => {
      ...
      return (
        parseColor(value, previous?.value.h ?? DEFAULT_COLOR.h) ?? previous?.value ?? DEFAULT_COLOR
      );
    },
  });
