...
@Component({
  ...
  template: `
    @if (showValue()) {
      <label [for]="valueInputId" hlmFieldLabel>Color value</label>
    }
    @if (showFormat() || showValue() || showOpacity()) {
      <div [class]="editorGridClass()">
        ...
        @if (showValue() || showOpacity()) {
          <div [class]="inputGridClass()" hlmButtonGroup>
            @if (showValue()) {
...
export class ColorValueEditorComponent {
  ...
  protected readonly editorGridClass = computed(() =>
    hlm(
      'grid min-w-0 gap-2',
      this.showFormat() && (this.showValue() || this.showOpacity())
  ...
  protected readonly inputGridClass = computed(() =>
    hlm(
      'grid w-full min-w-0',
      this.showValue() && this.showOpacity()
        ? 'grid-cols-[minmax(0,65fr)_minmax(0,35fr)]'
        : 'grid-cols-1',
    ),
  );
