...
export async function runCli(argv: readonly string[], runtime: CliRuntime = {}): Promise<number> {
  ...
  function createCommand(): Command {
    ...
    program
      .name('qrc')
      .description('Generate QR codes as terminal text, SVG files, or PNG files.')
      .argument('[data]', 'QR code input data')
      .option('--input <value>', 'QR code input data')
      .option('--format <format>', 'output format: text, svg, or png')
      .option('-o, --output <path>', 'output path for SVG or PNG files')
      .option('--mode <mode>', 'QR data mode: numeric, alphanumeric, or octet')
      .option('--error-correction <level>', 'error correction level: L, M, Q, or H')
      .option('--version <version>', 'QR version from 1 to 40')
      .option('--mask <mask>', 'QR mask from 0 to 7')
