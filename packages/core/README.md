...
If you do not provide a mode, the builder automatically splits the input into the shortest-bit
combination of numeric, alphanumeric, and octet segments. Setting a mode forces the entire payload
to use that single mode. Mixed encoding is automatic; the builder does not accept manual segment
arrays.

QRCodeSDK implements QR Code Model 2 matrix generation for numeric, alphanumeric, and UTF-8
octet payloads, including automatic mixed-mode encoding. Octet payload bytes are always encoded as
UTF-8. The UTF-8 ECI declaration is disabled by default. Kanji mode
...
| `.mode(mode)`                            | Forces `numeric`, `alphanumeric`, or `octet` mode.      | `Auto`   |
...
| `.eci(enabled?)`                         | Emits UTF-8 ECI assignment 26 for octet segments.       | `false`  |
...
`QRCodeMatrixOptions` accepts `eci?: boolean`. You can also enable it with `.eci(true)`:

```ts
qrcode('Grüße ✅').eci(true);
qrcode('Grüße ✅').config({eci: true});
```

Enabling ECI emits assignment 26 once, immediately before the first octet segment, so scanners
receive an explicit UTF-8 declaration. Leaving it disabled saves 12 bits and matches common QR
generators, but scanners may rely on heuristics for non-ASCII text. Numeric and
alphanumeric-only symbols are identical with either setting. No ECI header is emitted when the
generated symbol contains no octet segment.
