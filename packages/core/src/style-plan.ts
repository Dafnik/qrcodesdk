...
export function createQRCodeStylePlan(
  ...
  const finderDarkCells = createFinderDarkCellSet(finders);
  const primitives: QRCodeStylePrimitive[] = [];

  const isOrdinaryDark = (row: number, column: number): boolean =>
    !!matrix[row]?.[column] && !finderDarkCells.has(cellKey(row, column));
  ...
  for (let row = 0; row < moduleCount; row++) {
    ...
    for (let column = 0; column < matrixRow.length; column++) {
      ...
      primitives.push(
        createModulePrimitive(
          column,
          row,
          styling.margin,
          'dots',
          styling.dotsOptions.color,
          styling.dotsOptions.type,
  ...
  return {
    ...
    hasCurves: primitives.some((primitive) => primitive.shape !== 'square'),
    primitives,
  };
}
...
function createFinderDarkCellSet(finders: readonly Finder[]): Set<string> {
  const cells = new Set<string>();
  for (const finder of finders) {
    for (let row = 0; row < FINDER_SIZE; row++) {
      for (let column = 0; column < FINDER_SIZE; column++) {
        if (isFinderRingCell(row, column) || isFinderCenterCell(row, column)) {
          cells.add(cellKey(finder.y + row, finder.x + column));
        }
      }
    }
  }
